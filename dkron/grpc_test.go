...
func TestGRPCExecutionDone(t *testing.T) {
	...
	err = a.Store.SetJob(nil, testJob, true)
	require.NoError(t, err)
	...
	err = a.Store.SetJob(nil, testChildJob, true)
	require.NoError(t, err)
	...
	t.Run("Should run job", func(t *testing.T) {
		...
		execs, err := a.Store.GetExecutions(nil, "test", &ExecutionOptions{})
		require.NoError(t, err)
	...
	t.Run("Should run a dependent job", func(t *testing.T) {
		execs, err := a.Store.GetExecutions(nil, "child-test", &ExecutionOptions{})
		require.NoError(t, err)
	...
	t.Run("Should store execution on a deleted job", func(t *testing.T) {
		...
		_, err = a.Store.DeleteJob(nil, testJob.Name)
		require.Error(t, err)

		// Remove dependents and parent
		_, err = a.Store.DeleteJob(nil, testChildJob.Name)
		require.NoError(t, err)
		_, err = a.Store.DeleteJob(nil, testJob.Name)
		require.NoError(t, err)
	...
	t.Run("Test ephemeral jobs", func(t *testing.T) {
		...
		j, err := a.Store.GetJob(nil, "test", nil)
		assert.Error(t, err)
		assert.Nil(t, j)
	})
